...
import { ShoppingCart, Package, Upload, FileBarChart, TrendingDown, Map, ClipboardList, Layers } from 'lucide-react';
...
export function DashboardPage({ onNavigate }: DashboardProps) {
  ...
  useEffect(() => {
    (async () => {
      const tables = ['sales', 'purchases', 'closing_stock', 'outlets', 'items', 'area_managers', 'recipe_mapping'];
      ...
      setCounts({
        ...
        recipeMapping: (results[6].count as number) || 0,
      });
    })();
  }, []);
  ...
  const cards = [
    ...
    { label: 'Recipe Mapping Rows', value: counts.recipeMapping, icon: <Layers size={20} />, page: 'recipe-mapping' as PageId, color: 'bg-purple-50 text-purple-600' },
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            onClick={() => onNavigate('report-master')}
            className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-white rounded-lg text-sm hover:bg-slate-900 transition-colors"
          >
            <Layers size={16} />
            View Master Report
          </button>
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-5">
        ...
        <ul className="text-sm text-blue-800 space-y-1">
          <li><strong>Opening (Day-1 Closing)</strong> = Previous day's closing stock</li>
          <li><strong>Purchase (Day-1 Purchase)</strong> = Previous day's purchase</li>
          <li><strong>Closing (Day Closing)</strong> = Selected date's closing stock</li>
          <li><strong>Actual Consumption</strong> = Opening + Purchase - Closing</li>
          <li><strong>Ideal Consumption</strong> = SUM over every recipe row: Sale Qty (previous day) x Qty Used (g) / Unit of Filling (g)</li>
          <li><strong>Variance</strong> = Actual Consumption - Ideal Consumption</li>
          <li><strong>Expected Closing</strong> = Opening + Purchase - Ideal Consumption</li>
        </ul>
      </div>
    </div>
  );
