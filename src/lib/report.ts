...
export function buildReportRows(input: ReportInput): ReportRow[] {
  ...
  // Closing stays unique by (date, outlet, item) — single value per key.
  ...
  // Purchases and sales are transaction-level: multiple rows per
  // (date, outlet, item) are valid (separate POs / invoices), so sum them.
  ...
  for (const s of sales) {
    if (s.date === prevDay) {
      ...
      prevSales.set(key, (prevSales.get(key) ?? 0) + s.qty);
    }
  }
